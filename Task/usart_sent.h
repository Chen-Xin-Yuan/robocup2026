...
/* K230 上位机视觉通信（USART6, 115200，ASCII 以 '\n' 结尾，兼容原 K210 协议） */
...
/* USART6 上位机命令（固定帧格式，带长度字节，无校验）: AA <长度> <数据...> 0A
 * 上位机->车:
 ...
 * 接收处理位置: USART6_IRQHandler 里直接读 DR 后调用 Usart6_OnByte()。
 */
void Usart6_RxInit(void);
/* 由 USART6_IRQHandler 调用：喂一个接收字节进帧状态机（同时喂 K210/K230 ASCII 行解析） */
...
uint8_t Color_TakeNew(void);         /* 是否有新收到的颜色(1=有, 读取后清除) */
void Color_Process(void);            /* 主循环：按 COLOR_REQ_INTERVAL_MS 请求颜色，收到有效颜色后冷却 COLOR_COOLDOWN_MS 再请求，收满5个停止 */
void Color_SendRequest(void);        /* 请求上位机回传 1 个颜色(AA 03 0A) */

/* ==================== 对圆心（USART6，纠正帧 AA 08 <x:4B> <y:4B> 0A，无 yaw） ==================== */
...
extern uint8_t circle_speed_active;  /* 1=Circle_AlignBySpeed 速度模式运行中(收到完毕帧立刻停车), 0=未运行 */

/* ==================== 二维码（USART6，二维码帧 AA 04 <编号> 0A） ==================== */
